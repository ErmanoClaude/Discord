...
function isValidDisplayName(displayName) {
  ...
  if (!(displayName.length >= 2))
    errors.push("Display name must be minimum 2 characters");
  if (displayName.length > 20)
    errors.push("Display name maximum of 20 characters");

  if (
    !(validator.isAlphanumeric(displayName) || validator.isAlpha(displayName))
  ) {
